...
$(function() {
 // SVG to work with.  The vis <div> is defined in HTML
	...
	var setScales = function() {
		// xScale
		xMin = d3.min(data, function(d){return d.le_1960})
        xMax = d3.max(data, function(d){return d.le_1960})
        xScale = d3.scale.linear().domain([xMin, xMax]).range([0, width])
		// yScale	
        yMin = d3.min(data, function(d){return d.le_2013})
        yMax = d3.max(data, function(d){return d.le_2013})
